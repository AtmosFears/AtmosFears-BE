...
public class AirParticulatesConverter {
  ...
  public static AirParticulatesWindroseInfo convertToWindroseParameters(AirParticulates particulates, Particulate particulate) {
    ...
    long particulateRange;
    switch (particulate) {
      case NO2:
        particulateRange = Arrays.stream(NO2_MAPPING).asDoubleStream().filter(range -> particulates.getNO2() != null && particulates.getNO2() > range).count();
        break;
      case SO2:
        particulateRange = Arrays.stream(SO2_MAPPING).asDoubleStream().filter(range -> particulates.getSO2() != null && particulates.getSO2() > range).count();
        break;
      case PM10:
        particulateRange = Arrays.stream(PM10_MAPPING).asDoubleStream().filter(range -> particulates.getPM10() != null && particulates.getPM10() > range).count();
        break;
      case PM25:
        particulateRange = Arrays.stream(PM2_5_MAPPING).asDoubleStream().filter(range -> particulates.getPM25() != null && particulates.getPM25() > range).count();
        break;
      case O3:
        particulateRange = Arrays.stream(O3_MAPPING).asDoubleStream().filter(range -> particulates.getO3() != null && particulates.getO3() > range).count();
        break;
      default:
        particulateRange = Arrays.stream(CO_MAPPING).filter(range -> particulates.getCO() != null && particulates.getCO() > range).count();
    }
